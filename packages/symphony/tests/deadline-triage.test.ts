...
describe("exhausted deadline successor triage", () => {
  ...
  test("the issue facts, complete material ledger, and preserved branch commit are inherited verbatim", () => {
    const source = failed();
    const preserved = [{
      branch: "v4-preserved/v4-acme-repo-65-a3-5741b62",
    ...
    expect(proposal.contractMarkdown).toContain(preserved[0]!.branch);
    expect(proposal.contractMarkdown).toContain(preserved[0]!.commit);
