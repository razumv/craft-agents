...
function deadlineFailure(events: readonly MaterialEvent[]): MaterialEvent | null {
  const terminal = events.reduce<MaterialEvent | null>(
    (latest, event) => latest === null || event.sequence > latest.sequence ? event : latest,
    null,
  );
  if (!terminal || terminal.state !== "failed") return null;
  if (terminal.kind !== undefined && terminal.kind !== "failure") return null;
  return /^attempt failed: Craft run stopped at (?:context|turn)-deadline$/.test(terminal.message) ? terminal : null;
}

function exhaustedAttempt(events: readonly MaterialEvent[], failure: MaterialEvent, maxAttempts: number): boolean {
  const attempt = events.reduce<{ sequence: number; attempt: number } | null>((latest, event) => {
    if (event.sequence >= failure.sequence) return latest;
    const match = /^attempt (\d+) (?:atomically claimed|running)$/.exec(event.message);
    if (!match || (latest !== null && event.sequence <= latest.sequence)) return latest;
    return { sequence: event.sequence, attempt: Number(match[1]) };
  }, null);
  return attempt !== null && attempt.attempt >= maxAttempts;
}
...
export function proposeDeadlineSuccessor(
  ...
  const failure = deadlineFailure(source.events);
  if (!failure || !exhaustedAttempt(source.events, failure, workflow.scheduler.maxAttempts)) return null;
  ...
  const preserved = [...preservedBranches]
    .map((entry) => ({ ...entry }))
    .sort((left, right) => preservedAttempt(left.branch) - preservedAttempt(right.branch)
...
function preservedAttempt(branch: string): number {
  const match = /-a([1-9]\d*)-[0-9a-f]{7}$/i.exec(branch);
